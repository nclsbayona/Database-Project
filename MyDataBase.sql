...
CREATE OR REPLACE VIEW MULTAS_ANIO_MES AS (
   SELECT EXTRACT(YEAR FROM MULTA.FECHA) AS ANIO, SUM(MULTA) AS VALOR_TOTAL
   FROM MULTA
...
WHERE CO.ID=M.IDCONDUCTOR AND CA.ID=M.IDCARRO AND M.MULTA>0
...
*/
CREATE OR REPLACE VIEW NAME_D AS(
SELECT CO.NOMBRE AS NOMBRE,CO.CEDULA AS CEDULA,COUNT(M.MULTA) AS CANTIDAD_MULTAS
FROM CONDUCTOR CO, MULTA M
WHERE CO.ID=M.IDCONDUCTOR AND M.MULTA>0
GROUP BY CO.CEDULA, CO.NOMBRE
);
SELECT * FROM NAME_D
UNION 
SELECT CO.NOMBRE AS NOMBRE, CO.CEDULA AS CEDULA, 0
FROM CONDUCTOR CO
WHERE CO.NOMBRE NOT IN ( SELECT NOMBRE FROM NAME_D );
--CONSULTA 3
/*
...
--CONSULTA 4
WITH T AS (
SELECT  M.ANIO AS AÑO, SUM(M.VALOR_TOTAL) AS TOTAL_AÑO
FROM MULTAS_ANIO_MES_NOELECTRICOS M
GROUP BY M.ANIO
)
SELECT CAST(T.AÑO AS VARCHAR(10)) AS AÑO, T.TOTAL_AÑO AS TOTAL_AÑO
FROM T
UNION 
SELECT 'TOTAL' AS AÑO, SUM(T.TOTAL_AÑO) AS TOTAL_AÑO
FROM T;
--CONSULTA 5
WITH SIN_MULTA AS 
(
SELECT DISTINCT CO.NOMBRE NOMBRE
FROM CONDUCTOR CO, MULTA M
WHERE CO.ID = M.IDCONDUCTOR AND M.MULTA = 0
)
,
 LISTAM AS
(
SELECT CO.NOMBRE NOMBRE
FROM CONDUCTOR CO, MULTA M
WHERE CO.ID = M.IDCONDUCTOR
GROUP BY CO.NOMBRE
)
SELECT LISTAM.NOMBRE
FROM LISTAM
MINUS
SELECT SIN_MULTA.NOMBRE
FROM SIN_MULTA;
--CONSULTA 6.¿Cuáles son los conductores que tienen un promedio de multas por año mayor al promedio de multas general por año? Liste el nombre del conductor, el año, el promedio general del año y el promedio del conductor en ese año.

WITH NUM_P_A AS(
  SELECT C.NOMBRE AS NOMBRE, EXTRACT(YEAR FROM M.FECHA) AS ANIO, COUNT(*) AS MULTAS
  FROM CONDUCTOR C, MULTA M
  WHERE M.IDCONDUCTOR = C.ID
  GROUP BY C.NOMBRE, EXTRACT(YEAR FROM M.FECHA)
)
SELECT PROM_PERS_GENERAL.NOMBRE, P_M_A.ANIO, P_M_A.PROMEDIO_MULTAS_ANIO AS PROMEDIO_MULTAS_PERSONA_ANIO, PROM_PERS_GENERAL.PROMEDIO_ANIO AS PROMEDIO_PERSONA_GENERAL
FROM (
      SELECT NUM_P_A.NOMBRE, SUM(NUM_P_A.MULTAS)/COUNT(*) AS PROMEDIO_ANIO
      FROM NUM_P_A
      GROUP BY NOMBRE
     ) PROM_PERS_GENERAL,
     ( 
      SELECT EXTRACT(YEAR FROM M.FECHA) AS ANIO, COUNT(*)/(SELECT COUNT(*) FROM CONDUCTOR) AS PROMEDIO_MULTAS_ANIO
      FROM MULTA M
      GROUP BY EXTRACT(YEAR FROM M.FECHA)
     ) P_M_A,
     NUM_P_A
WHERE NUM_P_A.ANIO = P_M_A.ANIO AND NUM_P_A.NOMBRE = PROM_PERS_GENERAL.NOMBRE AND PROMEDIO_ANIO > PROMEDIO_MULTAS_ANIO
ORDER BY NOMBRE;
...
--CONSULTA 8. Produzca la siguiente tabla que muestra el valor total de las multas por año y mes para los carros eléctricos y no eléctricos:
...
WITH T1 AS(
SELECT MAMN.ANIO AS ANIO, MAMN.MES AS MES, NVL(MAMN.VALOR_TOTAL, 0) AS NO_ELECTRICOS, NVL(MAME.VALOR_TOTAL,0) AS ELECTRICOS
FROM MULTAS_ANIO_MES_NOELECTRICOS MAMN
FULL OUTER JOIN 
MULTAS_ANIO_MES_ELECTRICOS MAME
ON MAMN.ANIO = MAME.ANIO AND MAMN.MES = MAME.MES
), 
T2 AS(
SELECT T1.ANIO AS ANIO, T1.MES AS MES, T1.NO_ELECTRICOS AS NO_ELECTRICOS, T1.ELECTRICOS AS ELECTRICOS, T1.ELECTRICOS + T1.NO_ELECTRICOS AS TOTAL
FROM T1
)
(
SELECT CAST(T2.ANIO AS VARCHAR(10)) AS ANIO, CAST(T2.MES AS VARCHAR(10)) AS MES, T2.NO_ELECTRICOS AS NO_ELECTRICOS, T2.ELECTRICOS AS ELECTRICOS, T2.TOTAL AS TOTAL
FROM T2
UNION
SELECT 'TOTALES' AS ANIO, '-' AS MES, SUM(T2.NO_ELECTRICOS) AS NO_ELECTRICOS, SUM(T2.ELECTRICOS) AS ELECTRICOS, SUM(T2.TOTAL) AS TOTALES
FROM T2)
